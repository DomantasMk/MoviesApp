...
export default function DetailsTVScreen({ route, navigation }) {
  ...
  React.useEffect(() => {
    fetch(
      `https://api.themoviedb.org/3/tv/${route.params.id}?api_key=debfa0037881857def6d74b243adf4a3&language=en-US`
    )
      .then((res) => res.json())
      .then((result) => {
        setSeries(result);
      ...
      {series ? (
        <View style={styles.innerContainer}>
          <Image
            style={styles.poster}
            source={{
              uri: `http://image.tmdb.org/t/p/w500${series.poster_path}`,
            }}
          />
          ...
          <SeriesList
            ...
            url={`https://api.themoviedb.org/3/tv/${route.params.id}/recommendations?api_key=debfa0037881857def6d74b243adf4a3&language=en-US&page=`}
          />
        </View>
      ) : (
        <View>
